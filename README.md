...
- **설정 프리셋:** 송신 설정 상단에서 시험 유형·엔진 조합·지연 반영·HDTN 10개 값을 이름으로 저장합니다. 송신 서버 DB에 최대 5개를 공유하며, 불러오기 후 편집해 **변경 저장**하거나 **관리**에서 이름 변경·삭제합니다. 연결 주소·COM·입력 파일은 바꾸지 않고 시험도 자동 시작하지 않습니다. `/clear` 및 시험 보관기간 정리로 프리셋은 삭제되지 않습니다.
...
2026-09-22에 LANS·PocketSDR 50개 파일을 사용자 지정 `오픈소스` 폴더의 원본으로 다시 맞췄습니다. 기존 한글 설명은 `01-korean-comments.patch`로 보존하여 빌드 수정본에서 읽을 수 있습니다. 비교용 비트·LDPC 덤프와 전용 메모리·잠금·파일 생성은 제거했고, 오류·경고와 PVT 계산에 필요한 `$IQOBS`·`$IQAFS` 출력은 유지합니다.
