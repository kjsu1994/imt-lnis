...
- [어댑터 개발자 공유 계약 — API-SPEC 13.1~13.3](API-SPEC.md#13-dtnhdtn-어댑터-개발자-공유-계약)
...
수신 화면은 전송 JSON의 송신 기준 PVT와 독립 계산한 수신 PVT를 좌우로 비교합니다. 기준값이 없는 과거 시험은 비교 불가로 표시합니다. 송신 화면의 비교 결과는 기준 PVT 카드 하단에 표시합니다.
...
같은 입력으로 RAW와 AFS를 전송해 수신 화면과 비교하세요. 합성 GRAW는 다운로드 후 파일 적용도 가능합니다.
