...
수신 화면의 '수신 원문 기록'에서 거절·식별 불가 요청도 조회합니다. Docker stdout의 `DTN_SEND_BODY`와 `DTN_RECEIVE_BODY` BEGIN/END 사이에는 송신·검증 전 수신 본문이 기록됩니다. 인증 헤더는 출력하지 않습니다. 잘못된 UTF-8 바이트는 콘솔에서 대체 문자로 보일 수 있으며 정확한 바이트는 DB 원문 다운로드로 확인합니다.
